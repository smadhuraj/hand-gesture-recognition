...
class HandTracking:
  ...
  def mainFunction(self, cap, canvasMain, canvas, movment_lable):
    ...
    while (True):
        ...
        hands = self.hand_cascade.detectMultiScale(gray, 1.1, 3)  # find the hand object that has similer haar features like trained images
        for bbox in hands: 
            a = bbox[0]
            b = bbox[1]
            c = bbox[2]
            d = bbox[3]
            newBox = (a,b,c,d)
            self.boxes.append(newBox) #  if found some hand that append to the boxes list
